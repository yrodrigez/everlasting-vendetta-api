...
export class LinkOAuthAccountUseCase {
    ...
    async execute({
        ...
        this.logger.info(`Linking OAuth account for user ID: ${userId} with provider: ${provider}`);
        ...
        if(provider === 'discord_oauth') {
            ...
            const bnetUserInfo = await this.blizzardOAuthService.getUserInfo(accessToken);
        ...
        const oldOauthProvider = await this.authRepository.findUserByProviderUserId(providerUserId, provider as Provider);
        if (oldOauthProvider && oldOauthProvider.userId !== userId) {
            this.logger.info(`OAuth account already linked to another user ID: ${oldOauthProvider.userId}. Merging accounts.`);
        }
        ...
        this.logger.info(`Successfully linked OAuth account for user ID: ${userId} with provider: ${provider}`);

        if (provider.indexOf('bnet_oauth') !== -1) {
            this.logger.info(`Syncing Battle.net members for user ID: ${userId} after linking OAuth account.`);
            const characters = await this.syncBattlenetMembersToUserUsecase.execute(userId, accessToken);
            return {
                ...newOauthProvider,
                characters,
            };
        ...
        if (oldOauthProvider) { // this case is discord, we need to sync members from old oauth to new one because discord had members linked
            this.logger.info(`Syncing members from old OAuth provider for user ID: ${userId} after linking new OAuth account.`);
            const characters = await this.syncMembersToNewUserUseCase.execute(userId, oldOauthProvider.userId);
            return {
                ...newOauthProvider,
                characters,
            };
