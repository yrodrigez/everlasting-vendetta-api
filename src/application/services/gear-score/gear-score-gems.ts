const UNWANTED_GEMS = [
    ...
    28458, 28460, 28459, 28461, 28468, 28470, 28467, 28469, 28466, 28465, 28463,
    28464,
];
...
export const calculateIsFullyGemmed = (
    equipment: ItemWithGemsAndSockets[]
): boolean => {
    return equipment.every((item) => {
        ...
        if (hasGems && expectedGems === 0) {
            throw new Error(
                `Item has gems but no sockets, which is unexpected. Item: ${JSON.stringify(item)}`
            );
        }

        const onlyRelevantGems = (item.gems ?? []).filter(
            (gem) => !UNWANTED_GEMS.includes(gem.itemId)
        );
