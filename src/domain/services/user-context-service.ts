...
export class UserContextService implements IUserContextService {
    ...
    async getUserContext(userId: string, provider?: Provider): Promise<UserContext> {
        ...
        const characters = (await this.memberRepository.findAllByUserId(userId)).map(m => m.character);
        const isGuildMember = characters.some(char => 
            char.guild && GUILD_NAMES.includes(char.guild.name) &&
    ...
    private shouldRefreshToken(expiresAt: Date): boolean {
        ...
        const now = Date.now();
        const gracePeriodThreshold = now + UserContextService.REFRESH_GRACE_PERIOD_MS;
