...
export class JWTTokenService implements ITokenService {
    ...
    constructor(accessSecret: string | Buffer, refreshSecret: string | Buffer, jwtKid: string) {
    ...
    signRefreshFromJti({ jti, userId, family_id, provider, exp, }: { jti: string; userId: string; family_id: string; provider: Provider; exp: number; }): { token: string; expiry: number; jti: string; } {
        ...
        return { token: refreshToken, expiry: refreshTokenPayload.exp, jti: refreshTokenPayload.jti };
    }

    generateAccessToken(input: Partial<GenerateTokenPairInput>): { token: string; expiry: number; jti: string; } {
        const now = Math.floor(Date.now() / 1000);

        if (!input.userId || !input.roles || !input.permissions || !input.provider) {
        ...
        return { token: accessToken, expiry: accessTokenPayload.exp, jti: accessTokenJti };
    }

    generateRefreshToken(input: Partial<GenerateTokenPairInput>): { token: string; expiry: number; jti: string; } {
        const now = Math.floor(Date.now() / 1000);

        if (!input.userId || !input.provider || !input.familyId) {
            throw new Error('Missing required fields to generate refresh token');
        }
        ...
        return { token: refreshToken, expiry: refreshTokenPayload.exp, jti: refreshTokenJti };
    }

    generateTokenPair(input: GenerateTokenPairInput): TokenPair {
        const { token: accessToken, expiry: accessTokenExpiry, jti: accessTokenJti } = this.generateAccessToken(input);
        const { token: refreshToken, expiry: refreshTokenExpiry, jti: refreshTokenJti } = this.generateRefreshToken(input);
        return {
            accessToken,
            refreshToken,
            accessTokenExpiry,
    ...
    verifyAccessToken(token: string): AccessTokenPayload {
        return verify(token, this.accessSecret, { algorithms: ['HS256'] }) as AccessTokenPayload;
    }

    verifyRefreshToken(token: string): RefreshTokenPayload {
        return verify(token, this.refreshSecret, { algorithms: ['HS256'] }) as RefreshTokenPayload;
    }

    verifyAnonToken(token: string): AccessTokenPayload {
        return verify(token, this.anonKey, { algorithms: ['HS256'] }) as AccessTokenPayload;
    }
}