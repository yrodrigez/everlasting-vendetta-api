...
import {
    wowProfileCharactersSchema,
    WowProfileCharactersInput,
...
wowRoutes.post(
    createRoute<WowProfileCharactersInput>(
        {
        ...
        async ({ input: { access_token, realmSlug } }) => {
            ...
            const syncWowAccountCharactersUseCase =
                new SyncWowAccountCharactersUsecase(memberRepository);
            ...
            const characters = await getFullCharactersUseCase.execute(
                eligibleCharacters,
                bnetToken
            );
