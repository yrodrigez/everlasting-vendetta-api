...
export class CharacterReliabilityRepository implements CharacterReliabilityPort {
    ...
    async getMultipleCharactersReliability(
        characters: { characterName: string }[],
        realmSlug: string
    ): Promise<ReliabilityScore[]> {
        this.logger.info(
            `Fetching reliability scores for ${characters.length} characters on ${realmSlug}...`
        );
        const { data, error } = await this.databaseClient.rpc(
            "get_recent_raid_reliability_ratings",
            {
                p_character_names: characters.map((c) =>
        ...
        if (error) {
            this.logger.error(
                `Failed to fetch reliability scores for characters on ${realmSlug}: ${error?.message}`
            );
        ...
        const reliabilityScores: ReliabilityScore[] =
            data?.map((item: GetRecentRaidReliabilityRatingsResult) => ({
                characterName: item.character_name,
                finalRecentReliability: Number(
                    item.final_recent_reliability || 1
                ),
                ...
                opportunitiesConsidered: Number(
                    item.opportunities_considered ?? 0
                ),
                realmSlug: realmSlug,
            })) || [];
        this.logger.info(
            `Reliability scores fetched successfully for ${reliabilityScores.length} characters on ${realmSlug}.`
        );
        return reliabilityScores;
    }

    async getCharacterReliability(
        characterName: string,
        realmSlug: string
    ): Promise<ReliabilityScore> {
        this.logger.info(
            `Fetching reliability score for ${characterName} on ${realmSlug}...`
        );
        ...
        if (error) {
            this.logger.error(
                `Failed to fetch reliability score for ${characterName} on ${realmSlug}: ${error.message}`
            );
            throw new MemberRepositoryError(
                `Failed to fetch reliability score for ${characterName} on ${realmSlug}: ${error.message}`
            );
        ...
        this.logger.info(
            `Reliability score fetched successfully for ${characterName} on ${realmSlug}.`
        );
        return {
            ...
            opportunitiesConsidered: Number(
                data?.opportunities_considered ?? 0
            ),
            realmSlug: realmSlug,
        };
    }
