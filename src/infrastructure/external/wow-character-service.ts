...
export class WowCharacterService
    extends BlizzardApi
    implements IWowCharacterService
{
    async getCharacter(
        ...
        const isCharacterNameValid = /^\p{L}{2,12}$/u.test(
    ...
    async getCharacterAvatar(
        ...
        const avatarObject = (data?.assets ?? []).find(
            (asset: { key: string; value: string }) => asset?.key === "avatar"
        ) ?? { value: "/avatar-anon.png" };

        return avatarObject.value;
    }

    async getCharacterWithAvatar(
        realmSlug: string,
        characterName: string,
        token: string
    ): Promise<WoWCharacter> {
