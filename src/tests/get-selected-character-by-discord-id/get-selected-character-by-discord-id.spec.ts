...
describe('GetSelectedCharacterByDiscordIdUseCase', () => {
    ...
    const mockMemberRow = {
        ...
        character: {
            id: 123,
            name: 'Arthas',
            realm: { id: 1, name: 'Pyrewood Village', slug: 'pyrewood-village' },
            level: 60,
    ...
    it('should return the selected character when a valid discordId is provided', async () => {
        ...
        mockAuthRepository.findUserByProviderUserId.mockResolvedValue(oauthProvider);
        mockMemberRepository.findSelectedByUserId.mockResolvedValue(mockMemberRow);

        const result = await useCase.execute(discordId);

        expect(mockAuthRepository.findUserByProviderUserId).toHaveBeenCalledWith(discordId, 'discord_oauth');
        expect(mockMemberRepository.findSelectedByUserId).toHaveBeenCalledWith('user-uuid-1');
    ...
    it('should throw an error when no user is found for the discordId', async () => {
        ...
        await expect(useCase.execute(discordId)).rejects.toThrow('No user found for the provided Discord ID');
        expect(mockMemberRepository.findSelectedByUserId).not.toHaveBeenCalled();
    });
    ...
    it('should throw an error when no selected character is found for the user', async () => {
        ...
        await expect(useCase.execute(discordId)).rejects.toThrow('No selected character found for this user');
    });
});
